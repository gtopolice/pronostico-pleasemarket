...
export interface BinaryMarketCardPropsV2 extends MarketCardProps {
    market?: Market;
...
const AnimatedBinaryMarketImpl = (props: BinaryMarketCardPropsV2) => {
    // Default labels
    const defaultLabels = {
        yesLabel: "SI",
        noLabel: "NO",
    ...
    const displayProbability = React.useMemo(() => {
        ...
        const prob = props.selectedShare === "NO" ? 100 - mostRecentDate.y : mostRecentDate.y;
    ...
    const lgHeight = props?.market?.resolution_result !== null && props?.market?.resolution_result !== undefined ? "lg:h-[420px]" : props.selectedBinary ? "lg:h-[517px]" : "lg:h-[452px]";
    ...
    return (
        <div className={`w-full max-w-[424px] border border-[var(--outline-variant)] rounded-[12px] pb-4 flex flex-col gap-4 ${lgHeight} overflow-hidden`}>
            {/* Body */}
            <div
                            ...
                            <div className="flex flex-row items-center justify-between">
                                <div className="flex flex-row items-center gap-2">
                                    <span className="text-[12px] leading-[16px] tracking-[0.4px] font-[700]" style={{ color: probabilityColor }}>
                                        {displayProbability}% {l.probability}
                                    </span>
                                    <span className="text-[12px] leading-[16px] tracking-[0.4px] text-[var(--outline)] font-[400]">
                                        ${formatNumber(props.market?.volume || 0, 2)} {l.volume}
                                    </span>
                                </div>

                                {!props.isLoading && (
                                    props.selectedBinary ? (
                                        <IconButton onClick={(e) => { e.stopPropagation(); props.onClose() }} size="small">
                                            <CloseBtnIcon sx={{ fontSize: 20, color: "var(--primary)" }} />
                                        </IconButton>
                                    ) : (
                                        props.showSaveButton && (
                                            <SaveButton
                                                ...
                                                onlyIcon={true}
                                                activeColor="var(--save-button-bg)"
                    ...
                    {props.market?.resolution_result ? (
                        ...
                        >
                            ...
                            <Button
                                ...
                                label={`${props.selectedBinary === "yes" ? l.yesLabel : l.noLabel} ${props.isTradingLoading || !props.displayPrice ? "..." : `${Math.round(parseFloat(props.displayPrice) * 100)}¢`}`}
                                labelBottom={
                                    props.isPurchasing
                                        ? l.predicting
                                        : `${l.toWin} $${props.estimatedPayout}`
                                }
                                ...
                                disabled={
                                    ...
                                    !props.amount || parseFloat(props.amount) < 1 ||
                                    !props.isConnected ||
                                    parseFloat(props.estimatedPayout) <= 0 ||
                                    (props.userBalance ?? 0) < parseFloat(props.amount) || (Number(props.estimatedPayout) <= Number(props.amount) && Number(props.volume) >= 50)
                                }
                            />
                        </motion.div>
                    ) : (
...
export const AnimatedBinaryMarketV2 = React.memo(AnimatedBinaryMarketImpl, (prev, next) => {
    ...
    if (prev.displayPrice !== next.displayPrice) return false;
    if (prev.estimatedPayout !== next.estimatedPayout) return false;
